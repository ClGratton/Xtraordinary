...
bool JsonSettingsIO::saveSettings(const CrossPointSettings& s, const char* path) {
  ...
  // Companion-only control policy is not exposed in the on-device settings list.
  doc["companionPowerButtonHoldSeconds"] = s.companionPowerButtonHoldSeconds;
...
bool JsonSettingsIO::loadSettings(CrossPointSettings& s, const char* json, bool* needsResave) {
  ...
  s.companionPowerButtonHoldSeconds =
      std::min<uint8_t>(doc["companionPowerButtonHoldSeconds"] | static_cast<uint8_t>(1), 2);
