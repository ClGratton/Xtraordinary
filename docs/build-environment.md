...
The script refuses to start unless the firmware artifact, one ADB phone, and the requested serial port are present. It sends the debug app an explicit reset-preparation intent, requires confirmation that GATT was disconnected and closed, then stops the process. Before any application write, it uses esptool's read-only `read-flash` command on `otadata`, validates the OTA sequence CRC/state, and resolves the selected `app0` (`0x10000`) or `app1` (`0x650000`) partition from `firmware/partitions.csv`. An unknown or contradictory selection fails closed. It then writes and verifies only that selected application region, preserving NVS, pairing, `otadata`, and books, waits for boot, and relaunches the app. The read proves the selected boot target, not what ultimately ran after bootloader fallback; deployment remains incomplete until fresh capabilities/status/library/policy evidence carries the running firmware version.
...
```powershell
& .\scripts\verify-x3-reconnect.ps1 -Mode Background -Iterations 5 -TimeoutSeconds 20 -WakeViaUsbPort COM7
& .\scripts\verify-x3-reconnect.ps1 -Mode ProcessDeath -Iterations 3 -TimeoutSeconds 20 -WakeViaUsbPort COM7

# When real durable work is already queued instead of using a PC-attached wake:
& .\scripts\verify-x3-reconnect.ps1 -Mode Background -Iterations 5 -TimeoutSeconds 20 -ExpectedDiscoveryIntervalSeconds 30 -PersistentWorkAlreadyQueued
```

Background mode exercises the real `onStop` path and its clean disconnect. Process-death mode force-stops the app but waits beyond the X3's ten-second BLE supervision timeout before reconnecting; a three-second force-stop loop is invalid because it can race the still-live peripheral connection. Neither mode deletes the bond or toggles Bluetooth. The verifier requires an explicit discovery precondition: either real durable pending work, whose retry loop spans standby pulses, or a bounded recognized USB command that restores the generic fast-advertising window before each launch. A plain foreground launch is deliberately only a 15-second one-shot probe and may correctly miss a 1.5-second pulse on a 30-second cadence; treating its absence as a reconnect failure is invalid. A pass requires the Android 17/API 37 GATT path, status-0 connection, notification subscription, and a decoded X3 capabilities packet. One pass is not release acceptance. Also run at least three controlled X3 reset/reconnect cycles through the safe flash/reset workflow before promoting firmware.

With `-PersistentWorkAlreadyQueued`, `ExpectedDiscoveryIntervalSeconds` must match the applied X3 standby pulse setting. The verifier automatically allows that interval plus ten seconds of bounded GATT setup headroom even when `TimeoutSeconds` is smaller; a 20-second verifier must never fail a correct 30/60/120-second standby policy before its next permitted advertising pulse. With `-WakeViaUsbPort`, the recognized USB command restores fast advertising and the ordinary `TimeoutSeconds` applies.

For `Background`, the verifier waits for the real X3 LE ACL to close before relaunching. Do not replace this with the ViewModel's 1.5-second grace or another fixed delay: Android was physically observed to retain the underlying ACL for about five seconds, and a direct connection launched during teardown produced no callback. Every cycle must then show a fresh status-0 GATT, notification subscription, and `Capabilities`; a package registration or previously retained connection is not a pass. When no durable work exists, the app performs one 15-second foreground probe and may return to **Paired** before a 30/60/120-second pulse. Use this matrix after a real button/USB wake that rearms fast discovery, or while genuine pending work owns retry coverage; increasing only the verifier deadline does not extend the app's scan window.
