...
```powershell
& .\scripts\verify-x3-reconnect.ps1 -Mode Background -Iterations 5 -TimeoutSeconds 20 -ExpectedDiscoveryIntervalSeconds 30
& .\scripts\verify-x3-reconnect.ps1 -Mode ProcessDeath -Iterations 3 -TimeoutSeconds 20 -ExpectedDiscoveryIntervalSeconds 30
```
...
`ExpectedDiscoveryIntervalSeconds` must match the applied X3 standby pulse setting. The verifier automatically allows that interval plus ten seconds of bounded GATT setup headroom even when `TimeoutSeconds` is smaller; a 20-second verifier must never fail a correct 30/60/120-second standby policy before its next permitted advertising pulse.
