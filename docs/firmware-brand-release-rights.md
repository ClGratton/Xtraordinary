...
| XTEINK word mark and device representation | Uses `XTEINK` in setup, device status, firmware labels, compatibility copy, and a code-drawn X3/device outline. No repository-owned source file establishes a logo or product-image licence | `LegalDocuments.kt` identifies XTEINK as its owner's trademark and states that Xtraordinary is independent and unendorsed | Obtain a written permission/nominative-use decision for the exact store name, screenshots, word mark, device outline and compatibility wording. If permission is not obtained, remove logo-like/product-identity artwork and retain only counsel-approved compatibility text |
| XTEINK OEM firmware | Pins `XT V5.1.6 EN` to an XTEINK-hosted CDN object, downloads it into app cache, verifies fixed size/SHA-256 and flashes it | A public download URL and technical access are not evidence of permission to redistribute, deep-link, automate download, or install | Obtain written permission covering the in-app flow. Without it, public builds must not fetch the OEM object; replace the action with an official recovery-route link/instructions. Keep the fixed checksum for maintenance verification, not as a rights claim |
