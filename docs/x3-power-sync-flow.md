...
- USB book chunks use a transport-specific 240-byte payload rather than BLE's 488-byte maximum. Hex expansion plus the companion header and command prefix then occupy roughly half of the provisioned CDC RX queue, leaving sustained-frame scheduling headroom without weakening the shared protocol limit.
