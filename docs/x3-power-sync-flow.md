...
- The CDC receive queue is derived from the maximum companion envelope after hexadecimal expansion, plus command framing headroom. The framework's 256-byte default cannot hold a 512-byte envelope encoded as a roughly 1 KiB command; it is therefore not a valid transport limit.
- USB and BLE both use the protocol maximum of 488 data bytes per chunk. The CDC receive queue is sized from the resulting hex-expanded maximum envelope, so using a smaller hard-coded USB chunk only adds round trips and is forbidden.
- Begin carries a basename, exact size, and SHA-256. X3 accepts supported book extensions only, refuses traversal/separators, and writes to `/.crosspoint/companion/book-upload.tmp`. Retrying the same title is an atomic replacement, not an immediate rejection: the verified old file moves to a hidden file-specific backup only during publish, is restored if publish fails, and is deleted only after the verified temporary file owns `/Books/<name>`.
- X3 temporarily requests a fast BLE connection interval and holds full CPU clock only for the active transfer. Commit closes the file, recomputes SHA-256 from SD, publishes it atomically, rescans the library, then returns to the configured slow connection policy.
- The phone persists the ordered book-id queue and chosen transport before the first byte. After each successful commit it removes only that id from the queue. A transport interruption retains the remaining queue and resumes from byte zero when that transport returns. A device NACK/rejection, unreadable/empty source, validation failure, or explicit Stop is terminal: the queue is cleared and must never restart on foreground, reconnect, or process recreation.
- While a transfer is active, Android owns a foreground `dataSync|connectedDevice` service and the Bluetooth path remains persistent/fast even if the Activity is backgrounded. A sub-second background/foreground transition keeps the remembered device presented as **Paired** rather than flashing **Reconnecting**. Swiping the app task away is an explicit exit: it clears durable transfer intent, cancels the in-process owner, and stops the service.
- Import selection ends when transfer begins. Progress belongs to the matching book row; the stable bottom action becomes Stop. After a successful import the app immediately offers to send the one or more newly imported books through the same USB/Bluetooth chooser.
- **Stop upload** cancels and joins the active Android job, then sends `AbortBookUpload` in a non-cancellable cleanup context when transport still exists. A disconnect, explicit abort, inactivity timeout, size mismatch, hash mismatch, or write failure removes the temporary file. A partial or zero-chapter book is never exposed in the library.
- Import uses Android's EPUB MIME filter and validates that the OPF spine contains at least one readable XHTML item. Broad ZIP/octet-stream picker types are not used merely to compensate for mislabelled providers.
- An ACK for `CommitBookUpload`, `DeleteLibraryEntries`, or `GetLibrary` is not the library truth. Android subscribes before requesting the snapshot and retains the link until the final revisioned `LibraryPage` arrives; only that snapshot updates or clears the phone's X3 entries.
