...
- Fresh real Pixel evidence: Focus screenshot/XML and slider interaction prove a nonzero timer, Start focus, and responsive duration control; the test duration was finally restored to the measured pre-test 25:00 state in `pixel-dev51-focus-final-25.xml`/`.png`. Passes screenshot/XML prove the AHO→VCE card, Static/Live chooser, and Start & send action are above bottom navigation. These are product screenshots only; UI reviewer/screenshot-baseline verification remains explicitly deferred.
- Sequential firmware wrapper passed with the same narrow waiver. `xtraordinary-v0.2.6-dev51-maintenance-focus-passes-local` is 5,456,560 bytes, SHA-256 `465C5FB999B9B7E63926B0B91BAF895CFFFE843E0A31C130E8ACFB169708CB18`. Canonical resolver found exactly one Pixel but no X3 `VID:303A:1001`; dumpsys proves host-only/disconnected USB. No USB/stub/otadata/flash/reboot/lease or X3 data mutation occurred. Pixel timeout was restored and verified at `1800000` ms. Maintenance lease activation and fresh device protocol acceptance remain blocked on physical X3 enumeration.
...
- 2026-08-24 routing correction: Install was proven to reject LocalFile over BLE and choose raw USB based only on `usbConnected`; pushed `20d639b` now prefers the existing managed BLE client when fresh Capabilities support firmware updates, with focused transport-selection fixtures. The canonical focused Android wrapper did not reach Gradle: the task-local usage audit rejected a rollout resolved to another thread. No install or device mutation occurred.
- 2026-08-24 managed-BLE routing checkpoint: audit binding correction `e6c5562` prevents cross-task rollout attribution; focused Android protocol/unit/lint/assemble build passed. Retained-data Community reinstall completed with APK SHA-256 `843305CE1D9BF290A88772FE5DB49B2454A42B2C8DFE9CA8CDF9562426BB1180`. Fresh dev37 Capabilities and lease ACKs were seen, but the managed Install attempt emitted no BEGIN/chunk/commit/apply and no device mutation occurred. Timeout restored to `1800000` ms.
## 2026-08-25 - Explicit final build/install checkpoint

- User authorized the minimum final APK/firmware upload and requested no further agents or optional checks. The signed-in weekly meter reached 98%; the visible weekly-reserve override and 46-item UI-review-debt waiver were used only for one Community assembly from pushed `ac74f36`.
- Community APK SHA-256 `73E7F148F1152C065FF2BE394D34C701A3CAC5B4C9FB33B0B076E074677F8BB2` installed with retained data; Pixel package truth is versionCode 52/versionName 0.2.0-dev51. The exact already-accepted dev51 firmware SHA-256 `465C5FB999B9B7E63926B0B91BAF895CFFFE843E0A31C130E8ACFB169708CB18` was staged in Downloads but not redundantly flashed. Pixel timeout was restored to measured pre-test `2147483647`; no bond, app-data, NVS, SD, book, or X3 firmware state was cleared.
