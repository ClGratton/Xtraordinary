...
Expressive Focus keeps the duration deck on a pastel surface-container tint shared with the rest of the app. Strong primary color is reserved for active slider state and the main Focus action.
...
Search and sort operate on the merged phone and device catalog. Date, Name, and Size live in one mutually exclusive Sort dropdown; Service is a second dropdown, and device presence remains a toggle. These three controls fit in one row without duplicate sort chips. The device-presence chip uses the actual connected or last-known model name and filters only the authoritative device snapshot. All services and Local EPUB are available now, while future providers are visibly unlinked. A full-width dismissible Library services reminder below the books restores the readable card hierarchy of the former Research & sources surface and opens Settings. EPUB fields always win. A missing field may be enriched through a low-volume Open Library Search API request; normalized results and downloaded cover art are cached, and failed lookups are throttled for seven days. Enrichment state is deliberately not a user-facing filter.

Long-press selection does not replace or reflow the library. The Read heading and subtitle remain present; selected count and inline Select all occupy the same fixed-height slot as the folder action. Circular marks overlay the fixed-size covers. The book-plus action becomes trash at bottom right, and a single Clear all action appears at bottom left. Search, filters, card bounds, cover size, and metadata therefore remain at the same coordinates when selection starts.
