...
Passes and the XTEINK model picker render through one shared magnetic pager implementation. While the finger drags, supported Pixel-class devices use Android's documented `PRIMITIVE_LOW_TICK` pattern with intensity and cadence proportional to displacement before the magnetic threshold. Crossing the threshold in either direction emits the click again during the same touch. On the return stroke, the resisted offset is mirrored so the card trails the finger on the original side and progressively catches up near center. If a fast fling commits to another page before the dragged offset crosses that threshold, choosing the new pager target emits the same click immediately. The withheld drag distance eases out rather than teleporting, direct finger tracking resumes beyond the bump, and the settled card receives the separate visual wobble. Semantic haptic fallbacks cover devices without those primitives.
