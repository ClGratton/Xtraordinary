...
- [x] Implement one reusable transfer lifecycle in source through `e743bc8`: transport interruption retries; command/source/validation failure stops and forgets; explicit Stop/task removal cancels and joins; active work owns the foreground service and persistent fast transport. Physical acceptance remains pending.
...
- [x] Make the final revisioned library snapshot the source-level reconciliation boundary after upload/delete: `e743bc8` removes the optimistic USB/commit assignment and reconciles only from the returned snapshot. USB without a live BLE snapshot remains conservatively unreconciled until a later snapshot. Physical acceptance remains pending.
...
- [!] The candidate is pushed through `e743bc8`, including snapshot-truth reconciliation and bounded-milestone governance. The canonical Android/firmware/device stages are blocked at the 80% signed-in weekly reserve before their wrappers may start; separately, the canonical engineering gate has only the 27 user-deferred stale/missing `passes`/`android-ui` receipt failures. Do not install or flash an unreviewed candidate.
- [x] Add explicit non-default release-debt controls at `c3c4cf8`/`c09c6a7`: only the documented UI-receipt rule and weekly reserve may be waived; every other policy plus replay/task-growth safety remains fail-closed.
- [!] The first authorized build/deploy task stopped before compiler entry because replay median grew 93% to 84,602 tokens. No phone/X3 state changed. Continue this distinct milestone in a new user-visible task after compaction/fresh-context audit; do not rerun source fixes or UI reviews.
...
- [x] Physical acceptance: enter established deep sleep, wait about five seconds for sleep to settle, fast-tap Power, and confirm the X3 boots and remains awake. The owner accepted the behavior; the visible e-ink return is slightly slow and was not instrumented separately.
...
- [ ] Build/install Android dev36 and run the retained USB queue through the shared USB-availability drain. Capture `XteinkUsbBook` phase/offset evidence, then complete success, cancel/abort, stale-host restart-from-zero, and multi-file queue acceptance.
- [x] Install dev36 and prove the retained queue auto-resumes on USB availability. The X3 ACKed Begin in 87 ms, timed out on the first 488-byte chunk at offset 0, and ACKed Abort; inspection tied the deterministic failure to ESP32-C3 HWCDC's 256-byte default RX queue truncating the roughly 1,038-byte hex command.
- [ ] Build/flash firmware dev30 application-only with the generic CDC RX queue derived from `MAX_PACKET_BYTES`, preserving NVS/bond/SD, then rerun the same retained queue before the remaining cancel/restart/multi-file cases.
- [ ] Install Android dev37: the physical local-firmware flow proved a selected source card intercepted its nested **Install** action and reopened the document picker. Selected cards are no longer source selectors, leaving the contained install action authoritative.
- [ ] Install Android dev38: dev37 proved **Install** is now authoritative, then failed before erase/write because `prepareForExternalDeviceReset()` restarted the retained USB book queue and both operations claimed CDC. Route flash/reset through the exclusive USB-maintenance scope, pause rather than delete the queue, and block availability drains until maintenance releases ownership.
- [x] Build/install dev38, production-flash dev30 application-only, and verify the app-reported version. The flow entered ROM mode, wrote, verified on X3, and reported **Installed · restarting X3**; NVS/bond/app data/SD were preserved. After restart the retained 1,729-byte queue committed `/Books/RTL_test.epub`, cleared its durable ID, and the app library marked it on X3.
- [x] Install dev39 and retain the 68,277-byte queue after confirming the USB-specific 240-byte sustained-frame policy did not start: USB availability arrived before the persisted phone library was restored, and the empty-library guard had no later lifecycle event to retry it.
- [x] Build/install dev40, where restored-library readiness enters the same shared USB-work drain as attachment and foreground return. The retained queue automatically began, proving the initialization-order fix; Begin and the first 240-byte chunk ACKed, the immediately following chunk timed out at offset 240, and Abort ACKed five seconds later.
- [x] Build/install dev41 with a centralized 20 ms post-ACK CDC turnaround guard. It reproduced the exact timeout at offset 240, ruling out host-frame turnaround; the delayed Abort still ACKed and the durable queue remains intact.
- [x] Build/production-flash firmware dev31 application-only with one reusable bounded, non-blocking serial command accumulator. Exact embedded version and SHA-256 `DC1C925BC5EB244394838E46A81708913C47DD41B9527EBC2DDFEC42EDC6FEE0` were asserted immediately before the unique Install action; the app verified the write and reported **Installed · restarting X3** with NVS, bond, app data, and SD preserved.
- [x] Dev31 advanced the retained transfer from one 240-byte chunk to 26,400 bytes, then NACKed with Abort already inactive. Source tracing found the bounded idle BLE disconnect unconditionally aborted the shared `bookUploadActive_` transaction even when USB owned it.
- [x] Build/install Android dev42 without the disproven 20 ms guard and build/production-flash firmware dev32 with a transport-scoped book transaction owner. BLE disconnect may abort only BLE-owned work; USB does not request BLE-fast mode. Canonical Android and firmware builds passed from pushed commit `c9d86e0`; APK SHA-256 `8A255255093EB8C2E6860297C05C4DF87C6DB531B470E2AAA63F7116AB50191E`, firmware SHA-256 `7F28384C7C41798EDF09A3BDA9CCA20D6D0196EAF215D05D52E151A16F7A25EF`.
- [x] Replay the retained 68,277-byte USB queue after dev32. The durable queue cleared and the exact library record became `/Books/test_tables.epub`, proving a complete USB transaction and fresh-Begin stale-host/reboot recovery. Real books, app data, NVS, SD contents, and the Bluetooth bond were preserved.
- [x] Complete the two remaining USB-specific fixture cases: cancel/abort with no partial X3 entry, then sequential two-file queueing with both exact library records and an empty durable queue.
- [x] USB cancellation on dev42/dev32: the 68,277-byte fixture Begin ACKed, cancellation occurred during chunks at offset 480, Abort ACKed, the durable queue entry cleared by explicit user intent, and the refreshed library retained both fixtures as phone-only with no partial X3 entry.
- [x] Fix the sequential-queue retry exposed after cancellation. A two-file USB run and a clean-process replay both Begin-ACKed and accepted the first 240-byte chunk, then timed out waiting for the next reply at offset 240; Abort ACKed and both durable IDs remained. Dev33 routes ACK/NACK through a bounded reliable CDC reply primitive while leaving ordinary unattended logs on the 1 ms non-blocking path.
- [x] Canonically build and production-flash `xtraordinary-v0.2.6-dev33-usb-reply-local` from pushed commit `eb334e4`. The 72-rule policy gate and ESP32-C3 RV32IMC verification passed; firmware SHA-256 `09CFA412462129697839F889B0682CB89060B19AC6C6AD595934104F29BE9E84` was asserted in the picker and the app reported **Installed · restarting X3** after device verification.
- [x] Replay the preserved two-file USB queue after dev33. The individual post-restart frame logs completed before capture began, so no per-frame timing is claimed. Durable evidence is complete: both queued IDs cleared, the library became `4 books · 4 on X3`, and the exact restored fixture paths are `/Books/RTL_test.epub` and `/Books/test_tables.epub` while the two real books remained untouched.

## 2026-08-13 external Google, flight-update, and monetization scope recovery

- [x] Reconcile the attached run history with the authoritative product documents instead of treating remembered requests as implemented behavior.
- [x] Confirm the current Google reading-history scope: optional `drive.appdata` backup/merge/delete for reading sessions and minimum page time only. Books, covers, passes, Bluetooth state, firmware, and Gemini data are excluded; access tokens remain in memory.
- [ ] Configure the production Google OAuth client for the real package/signing certificate, then physically accept Drive create, merge/restore, delete, revoke, and reconnect. The current real-device boundary is Google Play services `DEVELOPER_ERROR`; source presence is not acceptance.
- [x] Confirm the current pass import scope: `.pkpass`, Google Wallet FlightObject JSON, save-JWT/shared-link input, and screenshot extraction are implemented. Google Wallet does not expose a consumer API that lists or exports arbitrary installed passes, and issuer private keys must not ship in the APK.
- [x] Implement the reusable live-flight client boundary: `FlightStatusProvider`, strict flight/date/origin identity matching, observed-at freshness, five-minute polling with thirty-minute failure backoff, durable snapshot merge, and acknowledged X3 replay through the generic interactive lease. Add arrival time and signed delay to Android and capability-gated ticket payload v2/X3 rendering; migrate persisted v1 X3 tickets. The injected HTTPS proxy endpoint remains unset until the external service exists, and credentials never enter the APK.
- [x] Recover the monetization contract: seven-day no-card ad-free trial; after expiry the full app remains functional with one anchored adaptive banner only on stable browsing surfaces; one-time lifetime ad-free purchase; no interstitial, app-open, or rewarded ads; community distribution contains no billing/ad SDKs and remains ad-free.

- [ ] Obtain the remaining external product contracts before claiming trial/ads production-complete: production identity/backend and Play Developer API verification for non-resettable same-identity entitlement, final Play product/test accounts, production AdMob identifiers, and published UMP consent configuration. Google Drive is not an Xtraordinary identity backend.
- [x] Implement the locally testable monetization core: centralized server-evidence entitlement reducer and injected clock, explicit unknown/offline-grace states, one banner decision policy, and separate Play/community build contracts. Community contains no billing/ad/consent SDK; the policy rejects ads on setup, pairing, discovery, transfer, firmware, reset, Google authorization, destructive, recovery, error, active-operation, trial, purchased, unknown, and non-browsing surfaces.
- [x] Add the Play development integration: Billing 9.1.0 purchase/restore/acknowledgement, UMP 4.0.0 consent/privacy options, Mobile Ads 25.4.0 anchored adaptive banner, Settings purchase surface, official Google sample ad identifiers in debug, and SDK-free/ad-free Community implementations. Non-debug purchases remain `Verifying` instead of granting entitlement without a backend.
- [x] Correct the reviewed monetization decision flow: add an explicit **Continue with free** exit, keep Restore/Privacy/GitHub as secondary roles with AA contrast, and add deterministic Expressive, 1.3x, and Quiet fixtures. The final pushed `a3ef139` candidate passed 122 policies, both variant compiles, both unit suites, and both screenshot-update suites; canonical assembly/install remains blocked on the complete final source-bound UI receipt set.
- [ ] Physically accept Play license-test purchase/restore, UMP consent/privacy options, and the official test adaptive banner from a Play-installed build; then implement and accept the authenticated entitlement backend, refund/revocation recovery, final product/AdMob identifiers, and production consent messages. Do not substitute a sideloaded billing failure, local timestamp, acknowledgement-only release entitlement, or mock banner for this acceptance.

## 2026-08-13 dev33 sleep/final-sync CPU lockup

- [x] Distinguish the observed X3 reset from an Android crash: the Pixel app remained alive and resumed, its crash buffer was empty, and Android exit history showed only intentional installer force-stops.
- [x] Retrieve the persistent dev33 crash report over the Pixel USB host without resetting the X3 or Bluetooth bond. It records a CPU lockup with no panic string immediately after a BLE reconnect and fast-radio restoration near the ten-minute inactivity deadline.
- [x] Treat saved SD/SPI and semaphore addresses as raw stack memory, not a backtrace; do not claim SD causality from them.
- [x] Capture retained runtime state through the generic Pixel USB path. It proves the crash report came from the preceding panic boot while the latest observed boundary completed as an ordinary deep-sleep reset (`reset=8`); the single-previous-boot trace cannot retroactively identify the older panic's last checkpoint.
...
- [x] Build the pushed `f31169d` correction through the canonical UI-evidence candidate: provenance, 97 policies, Community/Play compilation, both unit-test suites, and both screenshot suites passed; fresh Default and Quiet renders passed the narrow Terra affordance review. No phone install or X3 flash was performed.
- [x] Canonically build and application-only flash the final-sync USB/radio command-policy correction as `xtraordinary-v0.2.6-dev36-usb-radio-sync-local` from pushed `9378f4254b61c334eb0bf02649443ad595bf0fed`. The 98-rule pre-build gate, RV32IMC verification, 35.2% RAM / 83.0% flash build, guarded Android GATT release, application-only write verification, app relaunch, and fresh encrypted bootstrap passed. NVS, the Secure Connections bond, phone app data, and SD were preserved. Artifact size: 5,455,232 bytes; SHA-256: `15E79602C11E6D8736CD275A7D1EDDCD576E35DC7F69A7DBD98B3F3A8F3A6A5A`.
- [x] Canonically build and application-only flash `xtraordinary-v0.2.6-dev35-final-sync-local`. The 98-rule firmware-release gate, ESP32-C3 RV32IMC verification, and release build passed at 35.2% RAM / 83.0% flash. The guarded COM7 workflow received `PERIPHERAL_RESET_READY`, wrote only the application partition, verified the flash hash, relaunched Android, and preserved NVS, the LE Secure Connections bond, app data, and SD. Firmware SHA-256: `A8B157833668BD14A7022DECAC8B8D48C438DAAC2F04311B35A9A0B19AE0E9CC`. Post-flash RSSI -50 discovery, status-0 GATT, three services, MTU 256, notifications, Capabilities, StatusChanged, four LibraryPage replies, and both policy ACKs passed before clean idle release.
- [ ] Finish the dev36 sleep-boundary wake/readback acceptance before closing the final-sync incident.
  - Two separate retained snapshots report ordinary deep-sleep reset `8`; the latest crash readback has no new panic reason. This is evidence against a repeated CPU lockup, but it does not replace the remaining repeated physical wake/reconnect acceptance.
  - The reconnect harness waits for the actual X3 LE ACL to close before relaunching and requires a fresh capabilities handshake. It now also requires an explicit discovery precondition: real durable pending work or a bounded recognized USB wake. A plain foreground launch is only one 15-second probe and may correctly miss a 1.5-second pulse on the 30-second standby cadence. The 99th firmware policy rule prevents this false-failure regression.
  - The USB/radio lifecycle correction is physically accepted: after the initial USB/JTAG open reset to boot 4, two subsequent recognized `RUNTIME_TRACE` commands kept boot 4 unchanged; the next phone launch matched at RSSI -37 and completed status-0 GATT, notifications, and Capabilities. Reset-free valid USB activity therefore rearms Bluetooth as designed.
  - Current synchronized values are fast window 5 minutes, standby 30 seconds, connected background 2 seconds, sleep 10 minutes, full refresh 15 pages, and instant Power. With the app force-stopped and no traffic, COM7 disappeared at 630 seconds, directly observing deep sleep rather than an awake CPU lockup. Deep sleep also makes USB unavailable; a targeted Windows restart reported the device not connected. On the next physical Power press or cable replug, retrieve the retained trace for previous boot 4 and require reset `8` with no new panic marker, then run the repaired repeated wake/reconnect matrix.
- [x] Physically stress rapid Settings edits and finish at 5-minute sleep, 2-second connected background, and 15-page refresh. Android persisted only the newest desired values with `sync_pending=true`; the first link advertised at RSSI -52 but timed out with status 147 / HCI reason 0x08. One non-destructive Pixel Bluetooth off/on recovered the stack without deleting the bond. The durable intent replayed, X3 ACKed both radio and reader policy, `sync_pending=false`, Settings rendered **Synced to X3**, Android unregistered the GATT client, and the LE ACL closed with reason 22. Home standby-cadence observation remains part of the current sleep-boundary run.
...
- [x] Implement the two independent appearance axes in source. The draggable sea/astronaut picture carousel selects and previews light versus dark mode; the separate Settings choice selects Expressive versus Minimal; Minimal uses its own shapes, density/motion decisions, and visual treatment rather than only recoloring. Both choices persist independently.
- [ ] Physically accept the appearance axes on the Pixel: drag and tap both scenes, restart in each color mode, verify Expressive/Minimal independently, and record adaptive, reduced-motion, TalkBack, and restart evidence.
- [ ] Complete a product-wide legal and attribution audit before release: Android and firmware dependency notices, upstream reader-firmware obligations, redistributed fonts/icons/art, source/license offers, XTEINK name/logo/product-image and compatibility-claim permission, and user-facing Terms of Service plus Privacy Policy matching Google backup, live flights, entitlements, ads/consent, diagnostics, and account handling. Do not treat existing repository license files as product acceptance.
  - [x] Track the complete 25-file firmware notice pack and SHA-256 manifest for Xtraordinary/CrossPoint/CrossInk/Open X4, pinned embedded libraries, Expat/CC0, and redistributed fonts; run its verifier before every compiler through the engineering gate.
  - [x] Generate and enforce exact Community/Play release-runtime graphs (147/180 modules), POM licence hashes, 38 undeclared-POM overrides, full current open-source texts, Google terms pointers, and a manifest; preserve the notice trees in APK assets and byte-verify assembled artifacts.
  - [x] Bind all nine production/concept PNGs to hashes, dimensions, introducing commits, dates, and Git authors without inventing missing generation-rights evidence.
  - [x] Make public/release Android variants fail closed on the pinned XTEINK OEM image unless an explicit permission property is supplied; retain debug-only private recovery access.
  - [x] Remove decorative XTEINK text from ordinary setup, status, discovery, library, and Focus-preview UI; retain the name only for legal compatibility, the explicit OEM source, and the existing pairing/protocol identity.
  - [x] Generate a source/binary/notices/CrossPoint-baseline release record after every canonical X3 firmware build.
  - [ ] Resolve the confirmed ArduinoWebSockets LGPL static link; replace all artwork-provenance `MISSING` fields with evidence; obtain or remove XTEINK brand/product-art and OEM-image permissions; finalize publisher/legal identity and public policy URLs; close Play/AdMob/UMP/OAuth/flight-provider contracts and review the exact signed APK/AAB notice pack.
