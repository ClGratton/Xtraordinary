...
- [ ] Obtain the external product contracts before claiming trial/ads complete: production identity/backend and Play Developer API verification for non-resettable same-identity entitlement, Play product/test accounts, and AdMob/UMP identifiers and consent configuration. Google Drive is not an Xtraordinary identity backend.
...
- [ ] Add and physically accept the actual Play billing, entitlement-backend, UMP consent, and anchored-ad integrations after their external identifiers/accounts exist; do not substitute local timestamps or mock banners for this acceptance.
...
- [ ] Implement the two independent appearance axes correctly. The draggable sea/astronaut picture carousel selects and previews light versus dark mode. The separate Settings choice selects Expressive versus Minimal; Minimal must materially reduce density, decorative shape, motion, and visual noise rather than only recolor the same layout. Persist both choices independently and add adaptive, reduced-motion, TalkBack, and restart evidence.
