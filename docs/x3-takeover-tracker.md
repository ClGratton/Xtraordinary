...
- [ ] Complete the two remaining USB-specific fixture cases: cancel/abort with no partial X3 entry, then sequential two-file queueing with both exact library records and an empty durable queue.
...
- [ ] Fix the sequential-queue retry exposed after cancellation. A two-file USB run and a clean-process replay both Begin-ACKed and accepted the first 240-byte chunk, then timed out waiting for the next reply at offset 240; Abort ACKed and both durable IDs remain. Route ACK/NACK through a bounded reliable CDC reply primitive rather than the intentionally droppable 1 ms diagnostic-log path, build/flash dev33, and replay the preserved queue.
