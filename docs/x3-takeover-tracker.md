...
- [ ] Build/install dev41 with a centralized 20 ms post-ACK CDC turnaround guard. The invariant failure after exactly one chunk with both 488- and 240-byte payloads, followed by a successful delayed Abort, isolates host-frame timing from payload capacity; replay the retained 68,277-byte queue, then finish cancellation and multi-file acceptance.
