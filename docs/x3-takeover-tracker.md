...
- [ ] Implement live flight refresh behind the documented reusable `FlightStatusProvider` and a server-side proxy. FlightAware AeroAPI remains the quickest documented candidate; credentials must stay outside the APK. Add arrival time and signed delay only from provider evidence, not inferred ticket text.
...
- [ ] Implement and accept the centralized entitlement state machine and separate Play/community build contracts only against those decided external contracts; keep setup, pairing, discovery, transfer, firmware, reset, Google authorization, destructive, recovery, and error surfaces ad-free.
