...
- [ ] Prove repeated sleep-boundary reconnect and wake cycles on dev35 without a new CPU-lockup report, then retrieve retained diagnostics before closing the final-sync incident.
  ...
  - The reconnect harness now waits for the actual X3 LE ACL to close before relaunching and requires a fresh capabilities handshake. The previous fixed three-second wait raced Android's roughly five-second ACL teardown and could create a callback-less direct connection.
  - An awake 365-second runtime trace plus two full phone scan windows isolated a second lifecycle defect: valid USB commands extended the main inactivity deadline without rearming the companion advertising deadline, allowing an awake X3 with expired Bluetooth. Source now reuses `wakeFastAdvertising()` for every recognized USB command and policy-enforces that coupling. The correction remains unbuilt and unflashed because the signed-in weekly meter reached 92% used.
