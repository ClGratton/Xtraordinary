...
- [ ] Build/install Android dev42 without the disproven 20 ms guard and build/production-flash firmware dev32 with a transport-scoped book transaction owner. BLE disconnect may abort only BLE-owned work; USB does not request BLE-fast mode. Replay the retained queue, then finish cancellation and multi-file acceptance.
