...
- [ ] Install dev39 and replay the retained 68,277-byte queue from a fresh Begin using the USB-specific 240-byte sustained-frame policy. The first dev30 attempt proved one 488-byte chunk ACKed but the second vanished at offset 488; Abort ACKed and the durable queue remains intact.
