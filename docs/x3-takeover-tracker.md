...
- [ ] Build/production-flash firmware dev31 application-only with one reusable bounded, non-blocking serial command accumulator replacing per-frame `readStringUntil()` allocation/blocking. Preserve NVS, bond, app data, and SD, then replay the same retained 68,277-byte queue before cancellation and multi-file acceptance.
