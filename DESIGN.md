...
Light/Dark is directly manipulated in the existing Focus landscape artwork field: swipe the lighthouse/sea and astronaut scenes with the shared magnetic pager, and let its displayed position continuously carry the Material palette between the two endpoints. The settled page alone persists the color mode. Settings keeps a compact, ordinary text Light/Dark radio choice for accessibility and non-gesture selection; it contains no scene image carousel. Expressive/Minimal remains a separate axis. There is no Normal/Minimal tab, density mode, or separate Quiet screen.
...
- List items follow the same rule: insertion and Undo restoration expand/fade from the list seam, while removal slides and collapses into that seam. A row must never pop into existence or leave behind its action background after deletion.
...
- The Focus Light/Dark scene field is another use of that same pager, not a second drag implementation: its normalized displayed position interpolates the whole phone Material palette. Returning to origin, cancellation, fling, rotation, and Minimal reduced-motion all resolve from the same settled pager page; only that settled page writes durable `CompanionColorMode`.
