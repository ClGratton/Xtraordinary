...
The next firmware candidate is dev33. Physical USB cancellation passes on dev32, but the subsequent two-file queue and a clean-process replay both lose the second chunk reply at offset 240 while a later Abort still ACKs. Dev33 keeps ordinary serial logs non-blocking and moves only command ACK/NACK replies through a bounded reliable CDC transmit-and-drain primitive. The two durable fixture IDs remain queued for the post-flash replay.
...
- X3 application partition: `xtraordinary-v0.2.6-dev32-usb-owner-local`, built SHA-256 `7F28384C7C41798EDF09A3BDA9CCA20D6D0196EAF215D05D52E151A16F7A25EF`; production-phone-flashed application-only at `0x10000`, write verified, and restarted with NVS/bond/app data/SD preserved
