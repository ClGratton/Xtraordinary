# Xtraordinary release handoff — 2026-08-11

## Start here

The repository is on `main` at `8205f99` (`origin/main`) with an intentional, uncommitted release candidate spanning Android, protocol, firmware, tests, screenshots, and documentation. Nothing from this work has been committed or pushed yet.
...
- X3 application partition: `xtraordinary-v0.2.6-dev25-clock-local`, built SHA-256 `6D058C4A9385FE12CE91BE9D2CCEE2FAABFC3E6F9D0F7C6FCB4FD16043DDB09B`; PC-USB-flashed application-only at `0x10000`, write hash verified, and restarted with NVS/bond/SD preserved
...
- Xtraordinary firmware restore after stock: **complete**; device manager reports `Firmware: xtraordinary-v0.2.6-dev19-local`, `Paired`, and 98% battery
...
Firmware restore, fresh pairing, protocol readiness, and the deterministic Bluetooth-standby loop freeze are complete, but owner-visible button and deep-sleep wake acceptance remain open. Dev24's 80-byte RTC-retained trace proved the apparent input lock was actually the second 30-second standby pulse blocking in synchronous `advertising_->start()` at 92.444 seconds after idle had lowered the ESP32-C3 to 20 MHz. Dev25 centralizes the due-pulse lifecycle predicate and restores full clock before the vendor start. The canonical 40-rule build passed, the application-only flash was hash-verified, and the preserved bonded API 37 GATT/encryption/notification/capabilities handshake passed. In a 155-second single-session hardware run, trace loop counts advanced through 95, 125, and 150 seconds and returned to `COMPANION_LOOP_EXIT`, disproving the prior freeze. Do not call the entire sleep incident resolved until an announced physical sequence confirms ordinary buttons, manual **Sleeping** render, Power wake while USB-powered, and a fresh post-wake handshake. Build iteration was also corrected: candidate version is now generated in one `BuildVersion` object, reducing a version-only canonical rebuild from 7:19 to 54.34 seconds. The dated whole-app UI audit is `docs/ui-audit-2026-08-11.md`; the mandatory workflow is `docs/ui-decision-workflow.md`, and the build-blocking render contract is `docs/ui-surface-evidence.json`.
