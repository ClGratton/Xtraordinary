...
An unbuilt dev27/dev34 source candidate is in progress on the same branch. It retains the other agent's unfinished work: ACK-gated, persisted Focus commands; USB commands as device activity; and a debug-only production USB crash-report intent. It also fixes a newly reproduced dev26 gap: after the **Sleeping** frame appears, a second fast Power tap during the 1.2-second final-sync/2.1-second teardown window is currently lost because the main loop is blocked and deep sleep is not armed yet. The candidate makes that shared finalization lifecycle Power-interruptible, safely restarts if teardown already crossed its rollback boundary, and skips the unconditional 500 ms recovery-combination settle when an instant wake press is already released. This candidate must be committed and pushed before either canonical compiler may run.
...
- X3 application partition: `xtraordinary-v0.2.6-dev26-sleep-wake-local`, built SHA-256 `36088314D34DA6CD06315221F5E859E1565A7D9EF07C5F39DE2AED47F0D5AF0B`; PC-USB-flashed application-only at `0x10000`, write hash verified twice, and restarted with NVS/bond/SD preserved
