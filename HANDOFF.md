# Xtraordinary release handoff — 2026-08-12

## Start here

The complete accepted dev26 source is preserved on `codex/x3-dev25-recovery` at pushed commit `4d7ebaa3d5f9483e8593ce837153e17168d2febc`. Never compile a new candidate from uncommitted or unpushed source: both canonical wrappers invoke `scripts/assert-pushed-source.ps1` before the policy gate and compiler, and the check verifies a clean named branch, its upstream, matching local/upstream commits, and the live remote ref. Generated build output may be discarded; source checkpoints may not.
...
Firmware restore, fresh pairing, protocol readiness, the deterministic Bluetooth-standby loop freeze, and dev26 sleep/wake acceptance are complete. Dev24's retained trace proved the apparent input lock was the second 30-second standby pulse blocking in synchronous `advertising_->start()` at 20 MHz. Dev25 restored full clock before a due restart. Dev26 trusts the GPIO wake cause immediately for a zero-duration policy, renders **Sleeping** before a bounded 1.2-second final sync plus controller shutdown, removes the remaining pre-render synchronous stop, scopes retained tracing to X3, and keeps generated version state out of tracked source. The owner physically confirmed that the **Sleeping** frame remained visible throughout the final-sync interval and that a genuinely fast Power tap woke the X3 and kept it awake. After wake, the preserved Secure Connections bond opened a status-0 GATT link, discovered three services, negotiated MTU 256, subscribed to notifications, and received fresh `Capabilities` and `StatusChanged` messages. The phone required one Bluetooth off/on recovery after its native GATT client registered but produced no callback; the bond was not deleted.
