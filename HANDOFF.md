...
Pushed `ecc86d7` adds a visible top-left `Update available` status and gates managed BLE OTA to fresh Xtraordinary companion-family capability evidence; USB remains the initial/recovery path and stock/CrossPoint are never offered OTA. It also makes theme content polarity and system bars consume the same hysteretic boundary while retaining continuous palette interpolation. The one focused Android wrapper passed protocol tests, both flavor unit tests/lint, and assemblies with the explicit deferred UI-review debt waiver. Retained-data Community dev51 install succeeded: 86,612,485 bytes, SHA-256 `8136BF7A6346CCC0EF05EF7C4309D818B08587D33257DADED3A37CFB53706212`, versionCode 52. Live host USB is present; no USB-as-OTA workaround was used, so BLE-only OTA transfer/reboot proof remains pending a USB-absent managed link. Pixel timeout remains a protected device-phase state and must end verified at `1800000` ms. Screenshot/reviewer acceptance remains explicitly skipped.
...
The sequential firmware wrapper passed with the same narrow waiver, producing `xtraordinary-v0.2.6-dev51-maintenance-focus-passes-local`, 5,456,560 bytes, SHA-256 `465C5FB999B9B7E63926B0B91BAF895CFFFE843E0A31C130E8ACFB169708CB18`. Canonical target resolution found one Pixel and no present X3 `VID:303A:1001`; Pixel dumpsys USB proves `host_connected=true`, `connected=false`, `configured=false`, `kernel_state=DISCONNECTED`. No USB/stub/otadata/flash/reboot or X3 state mutation occurred. Pixel screen timeout was restored and verified at `1800000` ms. X3 maintenance lease was not activated because matching firmware could not be deployed. Remaining proof is physical X3 enumeration, guarded selected-slot deployment if available, then fresh Capabilities/StatusChanged/revisioned LibraryPage/policy ACKs and lease evidence.
...
The installed Passes evidence showed card bounds `[126,649][954,1699]` (400 dp) and chooser bounds `[42,1792][1038,2151]` (176 dp), with the selected action clipped by bottom navigation and large ownerless card space below tightly packed route facts. Pushed `4cb3504` uses actual remaining-height card budgeting through normal `font_scale=1.2`, keeps the normal chooser at 152 dp, and distributes the route, operational facts, identity/freshness, and face action across the card. Pure geometry checks cover 412x915, 360x800, and the measured Pixel scale.

Pushed `e48cc4a` adds a separate testing-only `AcquireMaintenanceLease` protocol primitive. Its 1-600 second deadline exists only in firmware RAM, zero seconds releases it, disconnect/expiry clears it, and it is never written to NVS or user settings. While active it parks only the inactivity/deep-sleep deadline; it neither requests transfer-fast BLE parameters nor holds the CPU at full clock. The debug app exposes the guarded maintenance intent, and `scripts/maintain-x3-device.ps1` brackets both the saved Pixel timeout and the renewable X3 lease with Begin/Restore/Run plus a mandatory `finally` restore. This replaces ad hoc physical wakeups during device testing once the matching app and firmware are deployed.

These are clean pushed source checkpoints only. No canonical Gradle build, retained-data install, post-fix screenshot, X3 firmware build/flash, or physical completion/radio acceptance has occurred yet. Pixel timeout was temporarily disabled for the active device-evidence phase from an original `1800000` ms and must be restored exactly when the phase ends.
...
The pushed source-matching Community dev50 APK assembled at `bfb088d` (85,625,905 bytes, SHA-256 `A5E21CD2A865A319791CC3C57531160152B1D4B5804A7F1ABA3F3E2317323D35`) and was installed retained-data; Package Manager reports versionCode 51/versionName 0.2.0-dev50, lastUpdateTime `2026-08-24 20:21:25`, and on-device base.apk SHA matches. The Android wrapper ended at known deferred screenshots; the sequential firmware wrapper failed before compiler on 9 UI-review policy violations, so no firmware artifact/USB deployment or X3 mutation occurred. Pixel timeout remains `1800000` ms.

The follow-up source pushed through `8697a9f` adds the canonical esptool C3 v2 stub asset (SHA-256 `47FD549A9746E6ACD3BE7C9F56BA71A02D9DAB9FF380501C4F71C79F762ABEF4`) and read-only otadata flow. Firmware wrapper passed with the narrow waiver, producing dev50 USB-first firmware (5,456,336 bytes, SHA-256 `743D3CC6BD9AE0BC54232807E439E701D864F5231427070097C59F7715030846`). The retained-data dev50 APK is installed, but target resolution is externally blocked: Pixel is ADB-reachable while dumpsys USB reports host-only/disconnected and no X3 VID:303A:1001 device. No USB mutation occurred; timeout remains `1800000` ms.
...
No new home card/row, artwork asset, shader, blur, firmware, OTA, tickets/device selector behavior, screenshot reference, phone, X3, pairing, app-data, NVS, or SD action belongs to this source checkpoint. The focused Community unit wrapper was first stopped before Gradle by the obsolete Settings-carousel rule. The user then explicitly selected the coherent alternative: Focus is the single scene carousel; Settings remains compact text-only. `DESIGN.md` and `docs/engineering-policy.json` were aligned to enforce that placement, shared pager reuse, displayed-position palette interpolation, settled-only persistence, and Settings radio semantics. The source-bound `android-ui` review record remains intentional deferred release debt; do not create a reviewer wave or regenerate screenshots in this usage-bounded milestone. The single permitted clean-pushed screenshot-free Community/Play unit/lint/assemble wrapper passed provenance, notices, usage, and the 174-rule engineering gate while explicitly deferring 55 UI-review findings, then started Gradle. Its final exit status was not retained by the shell session; the discovered APKs and XML results are older than the invocation and cannot prove compile/test completion. Do not retry this build in this milestone. No compiled candidate, UI acceptance, screenshot, install, or physical acceptance is claimed.
...
Durable diagnostic-order correction: powered-off/transport presence must be established before Android lifecycle attribution. Simultaneous BLE advertisement and USB enumeration absence is classified as X3 sleep/power-off or physical transport absence until wake and the reusable fast communication lease produce fresh evidence.
