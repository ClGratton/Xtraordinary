...
The Android dev35 app-only recovery correction is checkpointed at pushed commit `c4c273d`. Known phone transport blockers now outrank generic retry state, blocked retries pause, and foreground pending-work recovery caps the no-scan gap at 500 ms without making foreground visibility own GATT or X3 power. Dev38 is the current source candidate: dev36 added the shared retained-USB drain and phase/offset diagnostics; dev37 stopped a selected firmware card from intercepting its nested install action; dev38 gives firmware/reset maintenance exclusive USB ownership so a retained book queue cannot race ROM entry.
