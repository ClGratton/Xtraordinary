...
The complete dev25 recovery state is preserved on `codex/x3-dev25-recovery` at pushed commit `287c447`. Never compile a new candidate from uncommitted or unpushed source: both canonical wrappers now invoke `scripts/assert-pushed-source.ps1` before the policy gate and compiler. Generated build output may be discarded; source checkpoints may not.
...
- Xtraordinary firmware restore after stock: **complete**; the flashed application is `xtraordinary-v0.2.6-dev25-clock-local`; the stale dev19 device-manager observation predates that flash and is not current firmware evidence
...
Firmware restore, fresh pairing, protocol readiness, and the deterministic Bluetooth-standby loop freeze are complete. Dev24's retained trace proved the apparent input lock was the second 30-second standby pulse blocking in synchronous `advertising_->start()` at 20 MHz. Dev25 restores full clock before a due restart; a 155-second trace crossed repeated pulses, the owner physically confirmed standby navigation, and a later five-minute run rendered the explicit **Sleeping** frame and entered deep sleep. The owner then physically woke it with an approximately 500–600 ms Power press. A very fast tap still produced two Windows USB sounds and returned to sleep: zero-duration wake was incorrectly subjected to boot-time debounce after the tap had ended. The next candidate accepts the GPIO wake cause immediately for a zero-duration policy, renders Sleeping before a bounded 1.2-second final sync plus controller shutdown, removes the remaining pre-render synchronous stop, scopes retained tracing to X3, and keeps generated version state out of tracked source. Do not call full sleep acceptance complete until this candidate is built/pushed/flashed and both fast-tap wake and a fresh post-wake protocol handshake pass.
