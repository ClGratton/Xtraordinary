...
The dev27/dev34 source is checkpointed at pushed commit `7a9264c`. It retains the other agent's unfinished work: ACK-gated, persisted Focus commands; USB commands as device activity; and a debug-only production USB crash-report intent. It also fixes a newly reproduced dev26 gap: after the **Sleeping** frame appears, a second fast Power tap during the 1.2-second final-sync/2.1-second teardown window is currently lost because the main loop is blocked and deep sleep is not armed yet. The implementation makes that shared finalization lifecycle Power-interruptible, safely restarts if teardown already crossed its rollback boundary, and skips the unconditional 500 ms recovery-combination settle when an instant wake press is already released.
...
- Android installed candidate: `0.2.0-dev34` / code 35; canonical provenance, 54-rule policy gate, unit tests, lint, and debug assembly passed; APK SHA-256 `F68C9A27EB726F4CD66D8431E16A3E0E7D69EDF6C933B03B59D32BAF56CD3793`; installed with app data retained
- installed/source app: `0.2.0-dev33`, version code 34; APK SHA-256 `72F57F8E382A966FB838569DB86103771F8146151334763F6C5599A2C6A904BF`
...
- X3 application partition: `xtraordinary-v0.2.6-dev27-transition-wake-local`, built SHA-256 `343C4275A6755B8CDFDD393319A12873F98C48FEC81C142B9E8A304201F45198`; PC-USB-flashed application-only at `0x10000`, write hash verified, and restarted with NVS/bond/SD preserved
...
- Xtraordinary firmware restore after stock: **complete**; the flashed application is `xtraordinary-v0.2.6-dev26-sleep-wake-local`; the stale dev19 device-manager observation predates that flash and is not current firmware evidence
...
Firmware restore, fresh pairing, protocol readiness, and the deterministic Bluetooth-standby loop freeze are complete. Dev26 proved visible sleep and instant wake only after shutdown had settled; it did not cover a second press during the final handshake. The owner has now reproduced that transition gap, so the broader sleep/wake row is reopened until dev27 accepts both immediate transition cancellation and established-deep-sleep wake. Dev24's retained trace proved the earlier apparent input lock was the second 30-second standby pulse blocking in synchronous `advertising_->start()` at 20 MHz. Dev25 restored full clock before a due restart. Dev26 trusts the GPIO wake cause immediately for a zero-duration policy, renders **Sleeping** before a bounded 1.2-second final sync plus controller shutdown, removes the remaining pre-render synchronous stop, scopes retained tracing to X3, and keeps generated version state out of tracked source. After its accepted established-sleep wake, the preserved Secure Connections bond opened a status-0 GATT link, discovered three services, negotiated MTU 256, subscribed to notifications, and received fresh `Capabilities` and `StatusChanged` messages. The phone required one Bluetooth off/on recovery after its native GATT client registered but produced no callback; the bond was not deleted.
