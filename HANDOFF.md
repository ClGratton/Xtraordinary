...
The X3 has now moved from the PC's COM7 connection to the Pixel USB host, so PC PnP absence is expected and must not trigger any reset, re-pair, reflash, timeout change, or COM-port probe. The retained authenticated Pixel (`192.168.1.61:36221`) returned `1800000` for the timeout and the debug app's read-only USB diagnostic completed, proving Pixel USB-host communication. It reported `CrossPoint version: xtraordinary-v0.2.6-dev33-usb-reply-local`; therefore the prior dev37 image-write/hash record is not dev37 firmware-version acceptance. The retained LE Secure Connections bond remains present. No fresh BLE foreground `Capabilities`, `StatusChanged`, revisioned `LibraryPage`, or matching policy acknowledgement arrived, so the remaining physical proof is the complete BLE protocol-ready chain carrying the actual firmware version. No install, flash, reset, pairing/bond, app-data, NVS, SD, book, settings, or COM-port mutation occurred in this checkpoint.
