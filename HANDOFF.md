...
The appearance direction is not finished and has two independent axes. A draggable sea/astronaut picture carousel is the visual **light/dark mode** selector: dragging between the scenes previews and chooses the corresponding mode. The separate Settings choice is **Expressive/Minimal** and remains the concise, accessible control for visual complexity. **Minimal must actually reduce density, decorative shape, motion, and visual noise rather than merely recolor the same layout.** Both choices must persist independently and retain adaptive, reduced-motion, and accessibility evidence.
