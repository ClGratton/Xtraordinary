...
function Write-Utf8BomLines {
    ...
    # Windows PowerShell and PowerShell 7 disagree about whether
    # `-Encoding utf8` includes a BOM. The reviewed inventory has always used
    # one, so write it explicitly and keep the generated bytes host-independent.
...
function Write-LicenseMetadataReport {
    ...
    Write-Utf8BomLines -Path $Destination -Lines $lines
}
...
$manifestPath = Join-Path $noticeRoot $manifestName
Write-Utf8BomLines -Path $manifestPath -Lines @(Get-PackManifestLines)
