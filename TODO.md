...
- [~] Record exact app/firmware versions, commits, artifact hashes, install/flash results, retained-data checks, and the fresh capabilities/status/policy acknowledgement chain. The deployment record has dev49/dev37 artifacts, retained-data install/application-only flash, dev49/code 50, `1800000` ms timeout, app relaunch/process, retained X3 LE bond, and previous `VID_303A:1001`/COM7 evidence. Pixel-hosted read-only diagnostics now prove USB communication but report `xtraordinary-v0.2.6-dev33-usb-reply-local`, not dev37: treat the dev37 image hash as write evidence only. Fresh BLE `Capabilities` carrying the actual firmware version, `StatusChanged`, revisioned library, and policy-ACK evidence remains required; never treat USB diagnostics as BLE reconciliation.
