...
- [!] Rediscover the wireless-debug Pixel through the repository ADB/mDNS workflow and install with `adb install -r`, retaining app data. On 2026-08-24 the repository daemon found `_adb-tls-connect._tcp` at `192.168.1.61:42795`, but two connections to that current endpoint failed and `adb devices -l` remained empty; do not use a remembered port.
- [~] Compare the pushed firmware source with the installed X3 dev36 artifact. `firmware/src/companion/CompanionService.{cpp,h}` changed after dev36, so the canonical source-bound `xtraordinary-v0.2.6-dev37-book-reconciliation-local` build completed; its application-only flash is blocked until the guarded phone/device workflow is reachable. Preserve NVS, bond, and SD.
- [~] Record exact app/firmware versions, commits, artifact hashes, install/flash results, retained-data checks, and the fresh capabilities/status/policy acknowledgement chain. Artifact evidence is recorded; install/flash and fresh handshake evidence remain unavailable.
