...
- [!] Rediscover the wireless-debug Pixel through the repository ADB/mDNS workflow and install with `adb install -r`, retaining app data. On 2026-08-24 the repository daemon twice found `_adb-tls-connect._tcp` at `192.168.1.61:42795`, but connection failed and `adb devices -l` remained empty; the latest normal-user daemon restart found no mDNS services at all and again no devices. Do not use a remembered port, pair, or reset.
- [~] Compare the pushed firmware source with the installed X3 dev36 artifact. `firmware/src/companion/CompanionService.{cpp,h}` changed after dev36, so the source-bound `xtraordinary-v0.2.6-dev37-book-reconciliation-local` artifact (source `23acfd87919f178c2d1cf055ce1b2f311439f187`) is required. Its application-only flash is blocked until both the X3 `303A:1001` USB/JTAG target and guarded authenticated-phone release path are reachable; the latest PC scan found neither Espressif/X3 PnP target nor such a serial port. Preserve NVS, bond, and SD.
- [~] Record exact app/firmware versions, commits, artifact hashes, install/flash results, retained-data checks, and the fresh capabilities/status/policy acknowledgement chain. Exact artifacts are recorded at the deployment checkpoint; no install, flash, timeout query/change, retained-data check, or fresh handshake evidence is available.
