...
- [~] Record exact app/firmware versions, commits, artifact hashes, install/flash results, retained-data checks, and the fresh capabilities/status/policy acknowledgement chain. The deployment record has dev49/dev37 artifacts, retained-data install/application-only flash, dev49/code 50, `1800000` ms timeout, app relaunch/process, retained X3 LE bond, and previous `VID_303A:1001`/COM7 evidence. Pixel-hosted diagnostics are live USB communication, but the `dev33` version is a retained crash-report line, not an active firmware report; the active trace has no version. Treat dev37 hash verification as write evidence only and do not accept either version. Audit active `otadata`/boot-slot selection first: the current two-slot table has app0 at `0x10000` and app1 at `0x650000`, while the flasher writes app0 unconditionally. Fresh BLE `Capabilities` carrying the actual firmware version, `StatusChanged`, revisioned library, and policy-ACK evidence remains required.
