...
- [!] 2026-08-24 managed BLE Install retry: pushed `2c78ddd`/`34a3551` with reusable BLE firmware-install availability policy and focused tests; canonical Community/Play protocol/unit/lint/assemble wrapper passed. Retained-data Community APK installed successfully (dev49, 86,486,189 bytes, SHA-256 `9903D9F11E955CD54DF90AC5D797FF8AB3C976F26D2E7B257638427977FC3B26`). Fresh dev37 Capabilities, StatusChanged, revisioned LibraryPage, policy ACKs, and interactive-lease ACKs were observed. The Install surface became enabled while the link was ready, but the BLE session repeatedly ended before the final tap; the guarded fallback then displayed the no-OTA-slot notice and emitted no `BEGIN_FIRMWARE`. No transfer, reboot, flash, reset, bond/app-data/NVS/SD/book mutation occurred. Pixel timeout restored to `1800000` ms. Screenshots/UI verification remain explicitly skipped; next proof requires a stable link held through the final managed-OTA tap and fresh post-reboot acceptance.
...
- [!] One bounded managed-OTA retry after navigation-button wake again produced only `connect requested model=X3 phase=Disconnected` over ~75s; no BLE/USB transport event reached BEGIN. No Confirm timing retry or device mutation is applicable.
- [!] Android lifecycle diagnosis found adapter enabled, required scan/connect permissions granted, process alive, and idempotent scan cleanup already present; no scan callback or failure and no USB event occurred across a bounded interval. No source fix is justified without a fresh transport event.
- [!] Workflow correction: powered-off/transport-presence checks must precede Android BLE lifecycle diagnosis; the latest simultaneous BLE/USB absence is treated as X3 auto-power-off until wake plus fast-lease evidence.
