...
- [~] Record exact app/firmware versions, commits, artifact hashes, install/flash results, retained-data checks, and the fresh capabilities/status/policy acknowledgement chain. The deployment record has dev49/dev37 artifacts, retained-data install/application-only flash, dev49/code 50, `1800000` ms timeout, app relaunch/process, retained X3 LE bond, and previous `VID_303A:1001`/COM7 evidence. Fresh BLE `Capabilities` (including firmware version), `StatusChanged`, revisioned library, and policy-ACK evidence remains required. With X3 now USB-hosted by Pixel, prove USB detection separately and never treat it as BLE reconciliation.
...
- [~] Commit and push the current durable Windows ADB/X3 discovery workflow correction after its focused fixtures and engineering-policy validation pass.
